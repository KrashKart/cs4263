...
def build_search_representation(data: dict) -> dict:
    ...
    # convert goal states to tuple of tuples
    goals = tuple(map(tuple, data["goals"]))

    # grid is now a single integer with rightmost bit being (0, 0)
    initial_state = (grid, data["starting_position"][0], data["starting_position"][1], r, c, goals)
    ...
    def actions(state):
        ...
        for a in range(1, 5):
            ...
            if 0 <= newi < r and 0 <= newj < c and curr & (1 << (a - 1)) and _get_pos(grid, newi, newj) & (1 << (opps[a] - 1)):
                yield a
    ...
    def transition(state, action):
        grid, curri, currj, r, c, gs = state

        # if rotate, modify current cell and send back state
        if action == 5:
            new_grid = _rotate_pos(grid, curri, currj)
            return (new_grid, curri, currj, r, c, gs)
        
        # else, move the guy
        new_i, new_j = curri + didj[action][0], currj + didj[action][1]
        return (grid, new_i, new_j, r, c, gs)

    def is_goal(state):
        _, curri, currj, _, _, gs = state
        return (curri, currj) in gs

    # l1 distance will consistently underestimate/estimate exactly --> admissible
    def heuristic(state):
        _, curri, currj, _, _, gs = state
        return min(abs(curri - gi) + abs(currj - gj) for gi, gj in gs) * data["movement_cost"]
...
def search(problem: dict):
    ...
    """
    tiebreaker = 0
    h_start = problem["heuristic"](problem["initial_state"])
    frontier = [(h_start, tiebreaker, 0, problem["initial_state"])] # f = h, tiebreaker, g, state
    parent_state = {problem["initial_state"]: None} # for reconstructing path

    heapq.heapify(frontier)
    visited = {problem["initial_state"]: 0} # for lowest path cost (g) found for a state
    ...
    while frontier:
        ...
        if problem["is_goal"](state):
            path = []
            curr = state
            while parent_state[curr]:
                parent = parent_state[curr]
        ...
        for action in problem["actions"](state):
            ...
            if next_state not in visited or g + problem["action_cost"](state, action, next_state) < visited[next_state]:
                parent_state[next_state] = (state, action)
                tiebreaker += 1
                action_cost = problem["action_cost"](state, action, next_state)
                h_new = problem["heuristic"](next_state)
                heapq.heappush(frontier, (g + action_cost + h_new, tiebreaker, g + action_cost, next_state))
